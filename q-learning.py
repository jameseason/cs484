# James Eason
# CS 484 Assignment 3
# Windy Gridworld using Q-Learning
...
# Max reward that is attainable in the state following given state
def maxReward(s, r):
    rw = []
    if (s[0]+1, s[1]) in r:
        rw.append(r[(s[0]+1, s[1])])
    if (s[0]-1, s[1]) in r:
        rw.append(r[(s[0]-1, s[1])])
    if (s[0], s[1]+1) in r:
        rw.append(r[(s[0], s[1]+1)])
    if (s[0], s[1]-1) in r:
        rw.append(r[(s[0], s[1]-1)])
    return max(rw)
...
for x in range(50):
    ...
    while True:
        ...
        updateGrid(agentState, goal)
        if (agentState == goal):
            break
        nextAction = chooseAction(agentState, moveValues, e)
        q1 = round(moveValues[oldState][action], 3)
        q2 = round(moveValues[agentState][nextAction], 3)
        maxR = maxReward(agentState, r)
        moveValues[oldState][action] = q1 + lrate * ( r[agentState] + discount * maxR * q2 - q1)  
        moves += 1
